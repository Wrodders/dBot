...
int main(void){
    ...
    struct IMU imu = imuInit(IMU_A_ACCEL, IMU_A_GYRO,IMU_A_COMP, (IMU_FUSION_PERIOD * MS_TO_S));
    // MOTORS 
    ...
    motorEnable(&motorR);
    // BALANCE CONTROLLER
    struct PID balanceCtrl = pidInit(-RPS_MAX, RPS_MAX, BAL_KP, BAL_KI, BAL_KD, (BAL_CNTRL_PERIOD * MS_TO_S));
    pidSetRef(&balanceCtrl, -7.5f);
    // DIFFERENTIAL DRIVE MODEL (LinVel, AngVel)
    struct DDMR ddmr = ddmrInit(WHEEL_RADIUS, WHEEL_BASE, VEL_ALPHA, STEER_ALPHA);
    ...
    struct FixedTimeTask imuFusionTask = createTask(IMU_FUSION_PERIOD);     // Sensor Fusion 
    struct FixedTimeTask wspeedCntlTask = createTask(WSPEED_CNTRL_PERIOD);  // DC Motor Speed Control (PI)
    struct FixedTimeTask balanceCntrlTask = createTask(BAL_CNTRL_PERIOD);   // Balance Theta Control
    struct FixedTimeTask velCtrlTask = createTask(VEL_CNTRL_PERIOD);        // Linear Velocity Control 
    // ****** Loop Parameters **************************************************************************** // 
    uint16_t loopTick = 0;
	for(;;){
        loopTick = get_ticks();
        switch (state){
            ...
            case RUN:
                ...
                if(CHECK_PERIOD(balanceCntrlTask, loopTick)){
                    ...
                    motorSetTrgtSpeed(&motorL, balanceCtrl.out + steerCtrl.out); // Apply Steering Correction
