...
static uint16_t i2cReadSeq(uint32_t perif, uint16_t addr, uint8_t reg, uint8_t *data, uint16_t len) {
    //@Brief: Reads array of bytes from I2C Device
    //@Note: Assumes Device has Auto Increment Registers

    while ((I2C_SR2(perif) & (I2C_SR2_BUSY))); // Block if bus is busy

    i2c_enable_ack(perif); 
    i2c_send_start(perif); // Begin Select start register command

    // I2C_EVENT_MASTER_MODE_SELECT EV5: BUSY, MSL and SB 
    while (!((I2C_SR1(perif) & I2C_SR1_SB) & 
             (I2C_SR2(perif) & (I2C_SR2_MSL | I2C_SR2_BUSY))));

    i2c_send_7bit_address(perif, addr, I2C_WRITE);
    // I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED EV6: BUSY, MSL, ADDR, TXE and TRA 
    while (!(  (I2C_SR1(perif) & (I2C_SR1_TxE | I2C_SR1_ADDR)) & 
               (I2C_SR2(perif) & (I2C_SR2_MSL | I2C_SR2_BUSY | I2C_SR2_TRA)) ));
    (void)I2C_SR2(perif); // clear

    i2c_send_data(perif, reg);
    // I2C_EVENT_MASTER_BYTE_TRANSMITTED EV8_2: TRA, BUSY, MSL, TXE and BTF 
    while (!((I2C_SR1(perif) & (I2C_SR1_TxE | I2C_SR1_BTF)) & 
             (I2C_SR2(perif) & (I2C_SR2_MSL | I2C_SR2_BUSY | I2C_SR2_TRA))));


    i2c_send_start(perif); // Begin read command
    // I2C_EVENT_MASTER_MODE_SELECT EV5: BUSY, MSL and SB 
    while (!((I2C_SR1(perif) & I2C_SR1_SB) & 
             (I2C_SR2(perif) & (I2C_SR2_MSL | I2C_SR2_BUSY))));

    i2c_send_7bit_address(perif, addr, I2C_READ);
    // I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED EV6: BUSY, MSL and ADDR 
    while (!(  (I2C_SR1(perif) & (I2C_SR1_ADDR)) &
               (I2C_SR2(perif) & (I2C_SR2_MSL | I2C_SR2_BUSY)) ));

    uint16_t i;
    for (i = 0; i < len; i++) {
        if (i == len - 1) {
            i2c_disable_ack(perif);
            i2c_send_stop(perif);
        }
        /* I2C_EVENT_MASTER_BYTE_RECEIVED EV7: BUSY, MSL and RXNE */
        while (!(I2C_SR1(perif) & I2C_SR1_RxNE));
        data[i] = i2c_get_data(perif);
    }

    while ((I2C_SR2(perif) & (I2C_SR2_MSL | I2C_SR2_BUSY))); // Block if bus is busy

    return ++i; // return number of bytes read
}


static uint8_t i2cReadReg(uint32_t perif, uint16_t addr, uint8_t reg) {
    //@Brief: Reads one Byte from a I2C Device Register
    uint8_t data = 0;

    i2c_enable_ack(perif);
    i2c_send_start(perif);
    ...
    // I2C_EVENT_MASTER_BYTE_RECEIVED EV7: BUSY, MSL and RXNE 
    while (!( (I2C_SR1(perif) & I2C_SR1_RxNE )));
    data = i2c_get_data(perif);

    while ((I2C_SR2(perif) & (I2C_SR2_MSL | I2C_SR2_BUSY)));

    return data;
}


static void i2cWriteReg(uint32_t perif, uint16_t addr, uint8_t reg, uint8_t data) {
